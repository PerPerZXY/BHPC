...
from loss_functions.supcon_loss import SupConSegLoss, ContrastCELoss, BlockConLoss
...
def parse_option():
    ...
    parser.add_argument('--num_workers', type=int, default=16,
                        help='num of workers to use')  # 1000
    ...
    parser.add_argument('--block_size', type=float, default=16,
                        help='temperature for loss function')
...
def set_model(opt):
    model = SupConUnet(num_classes=3, mode=opt.head)
    if opt.mode == "block":
        criterion = BlockConLoss(temperature=opt.temp, block_size=opt.block_size)
    elif opt.mode == "stride":
        criterion = ContrastCELoss(temperature=opt.temp, stride=opt.stride)
    else:
