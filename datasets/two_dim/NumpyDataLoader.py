...
class NumpyDataSet(object):
    ...
    def __init__(self, base_dir, mode="train", batch_size=16, num_batches=10000000, seed=None, num_processes=0,
                 num_cached_per_queue=8 * 4, target_size=128,
        ...
        data_loader = NumpyDataLoader(base_dir=base_dir, mode=mode, batch_size=batch_size, num_batches=num_batches,
                                      seed=seed, file_pattern=file_pattern,
...
class NumpyDataLoader():
    def __init__(self, base_dir, mode="train", batch_size=16, num_batches=10000000,
        ...
        self.files, self.file_len, self.slices = load_dataset(base_dir=base_dir, pattern=file_pattern, slice_offset=0,
                                                              keys=keys, )
    ...
    def get_data_from_array(self, open_array):
        ...
        ret_dict = {'data': np.asarray(data), 'fnames': fnames,
                    'slice_idxs': slice_idxs}  # data_shape (8,1,64,64) 'data': np.asarray(data),
        if self.label_slice is not None:
            ret_dict['seg'] = labels
